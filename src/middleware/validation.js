...
const validateFarmerData = (req, res, next) => {
  try {
    ...
    const missingFarmerFields = requiredFarmerFields.filter(
      (field) => !farmer[field]
    );
    ...
    if (
      isNaN(parseFloat(farmer.user_latitude)) ||
      isNaN(parseFloat(farmer.user_longitude))
    ) {
      return res.status(400).json({
    ...
    for (const [index, farm] of farms.entries()) {
      ...
      const missingFarmFields = requiredFarmFields.filter(
        (field) => !farm[field]
      );
      ...
      if (
        isNaN(parseFloat(farm.farm_latitude)) ||
        isNaN(parseFloat(farm.farm_longitude))
      ) {
        return res.status(400).json({
    ...
    if (!req.params.id) {
      // Only check for required files during creation
      if (
        !req.files ||
        !req.files["farmer_picture"] ||
        !req.files["id_document_picture"]
      ) {
        return res.status(400).json({
